...
ax = plt.subplot()
plt.pie(ab_counts['email'], labels=labels, autopct="%0.2f%%")
plt.axis('equal')
plt.title('MuscleHub AB Test Split')
...
app_counts = just_apps.groupby(['ab_test_group','is_member']).email.count().reset_index()
member_pivot = app_counts.pivot(columns = 'is_member', index='ab_test_group', values='email').reset_index()
...
tmp = df.groupby(['ab_test_group','is_member']).email.count().reset_index()
final_member_pivot = tmp.pivot(columns = 'is_member', index='ab_test_group', values='email').reset_index()
...
plt.title('Percent of Visitors Completing Applications')
...
#Final Member Pivot Plot
plt.close('all')
plt.figure(figsize=(10,8))
ax = plt.subplot()

plt.title('Percent of All Visitors who Purchase Memberships')
